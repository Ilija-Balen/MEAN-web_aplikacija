...
@Component({
  selector: 'app-prva-klijent',
  templateUrl: './prva-klijent.component.html',
  styleUrls: ['./prva-klijent.component.css']
})
export class PrvaKlijentComponent implements OnInit {

  constructor(private router:Router, private adminservice:AdminsService, private userservice:UsersService) { }
  ...
  showazuriraj:boolean;
  show_azuriraj(){
    this.showazuriraj=true;
  }
  ...
  lastname:string;
  azurirajUser(){
    if(this.email==null && this.telephone==null && this.firstname==null && this.username==null &&
      this.lastname==null&&this.url==null){
        this.message="niste nista selektovali za promenu";
      }
    this.adminservice.azurirajUser(this.loggeduser.email, this.username, this.password, this.telephone,this.email, this.firstname,
