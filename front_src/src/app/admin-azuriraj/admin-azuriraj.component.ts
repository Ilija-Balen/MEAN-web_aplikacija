import { Component, OnInit } from '@angular/core';
import { AdminsService } from '../services/admins.service';
import { UsersService } from '../services/users.service';
import { User } from 'src/model/user';

@Component({
  selector: 'app-admin-azuriraj',
  templateUrl: './admin-azuriraj.component.html',
  styleUrls: ['./admin-azuriraj.component.css']
})
export class AdminAzurirajComponent implements OnInit {

  constructor(private adminservice:AdminsService, private usersservice:UsersService) { }

  ngOnInit(): void {
    this.emailSession = sessionStorage.getItem("korisnik");
    sessionStorage.removeItem("korisnik");
    this.usersservice.getUser(this.emailSession).subscribe((kor:User)=>{
      this.loggedUser=kor;
    })
  }

  emailSession:string;
  loggedUser:User;
  message:string;
  
  username:string;
  password:string;
  password2:string;
  telephone:number;
  email:string;
  firstname:string;
  lastname:string;
  agname:string;
  state:string;
  city:string;
  street:string;
  snumber:number;
  PIB:number;
  desc:string;

  azurirajUser(){
    this.adminservice.azurirajUser(this.emailSession, this.username, this.password, this.telephone,this.email, this.firstname,
      this.lastname, this.url).subscribe(msg=>{
        this.message = msg["message"];
        this.username=null; this.password=null; this.telephone=null; this.firstname=null;
      this.lastname=null; this.email=null;
      this.url=this.pocetniurl;
      })
  }
  azurirajAgency(){
    this.adminservice.azurirajAgency(this.emailSession,this.username, this.password, this.telephone, this.email,
      this.agname, this.state, this.city, this.street, this.snumber, this.PIB, this.desc, this.url).subscribe(msg=>{
        this.message = msg["message"];
        this.username=null; this.password=null; this.telephone=null; this.email=null;
        this.agname=null; this.state=null; this.street=null; this.PIB=null; this.desc=null;
        this.url=this.pocetniurl;
      })
  }
  
  selectFile(event: any) { 
		var mimeType = event.target.files[0].type;
		
		if (mimeType.match(/image\/*/) == null) {
			this.message = "Samo slike su podržane";
			return;
		}
    
   /* if(event.target.files[0].width>300 || event.target.files[0].height >300){
      this.message = "SLIKE VECE OD DOZVOLJENOG";
			return;
    } naturalWidth, naturalHeight*/

		var reader = new FileReader();
		reader.readAsDataURL(event.target.files[0]);
    
		reader.onload = (_event) => {
			this.message = "";
      this.url = _event.target.result;
    }
  }

  url:any ="data:image/png;base64,"
  pocetniurl = "data:image/png;base64,"
}
