...
export class AgencyComponent implements OnInit {
  ...
  constructor(private userservice: UsersService, private router:Router, private adminservice:AdminsService) { }

  ngOnInit(): void {
    this.loggeduser = JSON.parse(sessionStorage.getItem('loggeduser'));
    this.userservice.getUser(this.loggeduser.email).subscribe((kor:User)=>{
      this.loggeduser=kor;
  ...
  show_treca(){
    this.router.navigate(['treca-agencija']);
  }
  ...
  azurirajAgency(){
    if(this.email==null && this.telephone==null && this.PIB==null && this.agname==null && this.username==null &&
      this.state==null&&this.city==null&& this.street==null&&this.snumber==null&&this.PIB==null&&this.desc==null&&this.url==null){
        this.msgpass="niste nista selektovali za promenu";
      }
    this.adminservice.azurirajAgency(this.loggeduser.email,this.username, this.password, this.telephone, this.email,
      this.agname, this.state, this.city, this.street, this.snumber, this.PIB, this.desc, this.url).subscribe(msg=>{
        this.msgpass = msg["message"];
        if(this.email!=null){
          sessionStorage.clear();
          this.router.navigate(['']);
        }
